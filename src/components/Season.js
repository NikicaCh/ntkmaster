...
let urlProd = "https://xlsx-populator.herokuapp.com/";
let theUrl = urlProd;
...
class Season extends React.Component {
    ...
    dailyReport = async() => {
      ...
      .then((response) => { 
        if(response.status === 201) {
            setTimeout(() => {
                Axios.get(`${theUrl}download`)
                .then(response => {
                    window.location.href = "data:" + XlsxPopulate.MIME_TYPE + ";base64," + response.data;
                })
            }, 500) 
        }
      }).catch((err) => {
    ...
    deleteFromDb = (id) => {
        firebase.firestore().collection("Bolla").doc(id).delete().then(function() {
